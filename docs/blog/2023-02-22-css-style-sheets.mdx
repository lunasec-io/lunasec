...
If we want to say... turn a stylesheet off, then one way would be to get a reference to by the `document.styleSheets` property. This function can find a stylesheet
...
In hindsight, it might be possible to use the CSSStyleSheet API to look for some special rule you know will be there, or make a dummy rule for that purpose, but I didn't try that.
