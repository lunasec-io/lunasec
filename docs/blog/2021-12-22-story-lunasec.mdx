...
From the beginning, we have been translating Log4j updates and security community discourse into actionable advice and
[tools](/blog/log4j-zero-day-mitigation-guide#automatically-scanning-your-pack) which
automatically fix Log4Shell. With our decades of security
and development experience, we understood the implications of this vulnerability and gave this vulnerability
its name in hopes to coalesce and drive focused remediation efforts for companies. We responded to countless
security reports throughout our careers, big and small, and we know that if you are currently responding to Log4Shell, you
are most likely in a tight spot.

It is distressing, but easy to imagine, walking into a meeting, with the current state of your tech stack,
 and not being able to tell your boss that your company's data is _fully_ secure or compilant. Or even worse,
you may not be able to even confidently claim whether or not an attack has happened at all.
While everyone has recently been having these uncomfortable meetings about security, there has never been a better time
...
So now we are building Secure Functions, which look and act like normal functions in your server code, but in fact run in an ephemeral, sandboxed environment.
...
```
These functions are seamlessly deployed along with your actual application, requiring only a few changes. Being deployed to a hardened environment,
Secure Functions immediately reduce the impact of even a full RCE. Having a security team review these functions is still a good
practice to have, but it's much easier to identify and protect these sensitive data bottlenecks than an entire application.
