...
An emerging market of tools has become available for companies to use to "secure" their data. The most popular of these
...
but ideally a solution that could store *any* data wouldn't be so obvious to the user. The iFrame we created automatically disguises itself to 
blend into any app. With [a few changes](https://www.lunasec.io/docs/pages/overview/introduction) any form you have which collects sensitive data
can immediately become compliant, without compromising on security, all while looking like a native part of the web page.
