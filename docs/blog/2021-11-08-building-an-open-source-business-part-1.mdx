...
I followed while building [LunaSec](https://github.com/lunasec-io/lunasec). By comparison to traditional business 
...
you understand the steps involved, the tradeoffs you make by choosing Open Source, and overall by giving you an 
intuition to help evaluate if Open Source is the right approach for your business.

This series will be split up into an array of different posts, each one focusing on a different part of the process
involved with building and launching an Open Source business. In this post, I'll be explaining the evolution of software
pricing models, the pros and cons of each, and then relating them back to Open Source.

I hope this article helps you kickstart your efforts for launching your Open Source business. At the end of this series,
...
pricing models like hourly cloud-based software, per-seat subscriptions, and custom "Enterprise" contracts.
...
- **Brick-and-Mortar**: A traditional approach to software development, where you purchase a license, and pay for the
  software. This is old-school, and rarely used nowadays.
...
- **Cloud-Based**: An approach to software development where you purchase a license and pay for the software, but also
...
- **Open Source**: Distributed for free with no licensing or payment required. Software is delivered without any support
  ...
  Source, it must not be locked to a specific customer, and it must be able to be modified, improved upon, and resold by anyone 
...
* You sold individual copies of it on the shelf at a store or via the mail.
...
happened like they would for any physical goods. You bought a hammer just like you bought a copy of Microsoft Word by simply
...
*First*: It's expensive to scale. You have to pay to build a hammer every time you want to sell one. That means that to sell
...
Over time, the problems with the physical "Brick-and-Mortar" method of selling software forced a new pricing
model to be adopted. This new model is often referred to as the "Software Licensing" pricing model.

The idea is still fairly simple- instead of selling software off the shelf, software companies now started selling codes
that allowed you to use the software. Possession of the code was all that a user required to authorize them as the owner
of the software. If a code was shared too many times, the code would be considered "stolen", the license would be revoked,
...
download software from the internet, try the software for free, and only buy a license if you liked the software, or needed more features.

This didn't get rid of all piracy (consumers could still just use their friend's keys and call in as their friend), but
it did remove most of the financial burden for them to provide support to pirates. As a second-order effect, it also
...
for each computer. Companies could no longer just buy one copy for everybody to use.
...
As companies continued to grow and rely more heavily on technology, their needs became more complex. The software
...
Their customer's demands were not always the same, and the software companies had to adapt their pricing models
accordingly. A blanket policy would no longer suffice. The software companies would simply be leaving too much revenue
on the table. Features that were expensive to implement, or that were not commonly used, would need to be priced higher than
...
Over time, for many industries, software companies would begin to fill every niche. Some of these companies would
cater to a small number of very large customers, often referred to as "Enterprise" customers. They would create
specialized sales teams and onboarding processes that would be tailored to the Enterprise customer's needs. Any
companies _below_ a certain size would simply be turned away if they were unable to pay the exorbitantly high
prices.
...
be founded. Their solution to this divide (of software companies only wanting to sell to large customers), was to publish their software
...
that was needed to run it. This hardware was often expensive to produce, setup, and maintain, so buyers would often end
up renting it from the software companies instead.

The companies who wrote the software, managed the hardware to run it, and sold them bundled, would become known as the
...
Modern software licensing would no longer be just selling software, but also selling hardware, and led to a new class of
...
Snapchat, have been using cloud services scaled to meet consumption needs that vary over time. This consumption-based
...
Since the earliest days of computers, there has always been a contingent of software professionals that advocated for
...
top of. This approach flies in the face of the traditional beliefs held about software. That is, for the majority of
...
Red Hat realized, and subsequently taught the world, that the Source Code behind the software was not what was valuable, but
that it was instead the deep knowledge possessed by the people that created it that was valuable. That deep knowledge was necessary to fix
bugs in the software quickly, to save time setting up and maintaining the software, and to continue developing the
software to make it better over time by identifying patterns across the many ways that the software was used.

This insight that the "secret sauce" was not the Source Code, but instead was the knowledge of those that created it, was a
powerful tool for the software companies that wielded it. They were able to create a business model that was not only
profitable, but that also a greatly simplified adoption of the software. Companies leveraging Open Source would be able to
...
given non-editable binary files that they couldn't modify or extend, and getting approval from the Finance department and
...
easily, and eventually they would rely so greatly on that software that they would be happy to pay for it whenever something went wrong, when they
needed a feature, or when they needed support to ensure the software was working as expected.
...
In order to mitigate that risk however, there are a number of approaches that attempt to bridge the gap between the
proprietary and Open Source models by changing the rules of the license to be less "free". These approaches are generally 
...
prevent other companies like Amazon from deriving value purely from hosting a managed version of the software. 
...
licensing model to achieve traction and explosive growth, but still failed to create profitable business models despite
what would otherwise have seemed like a 
...
Beyond that cautionary note though, I think that there are a few places where Open Source really shines:
...
- **Developer Tooling**: If you're building a tool that will be used by developers, you probably want to use Open
  Source. Developers are a tricky market to sell to because they're often happy to build their own solution if
  the alternative is proprietary
...
- **Security or Compliance**: These areas are a bit hairier to understand where the right choice lies. In general, if
  ...
  compliance requirements by allowing them to self-host it. For HIPAA for example, it's a lot easier to self-host than it is
...
- **How do you make money?**: If you're planning to host the software for your customers, then you probably only want to
  ...
  picks up your software and starts hosting it, if you're not losing out on any profits, then what does it matter? You might
...
- **Is Open Source your only customer acquisition strategy?**: If the answer is yes, then you're in for a world of hurt.
  Open Source is frequently _more_ work than a proprietary solution when it comes to getting your first paying customers. It
...
----------
_Credits:_
- Photo of a computer programmer from the 70s from [here](https://media-bell-labs-com.s3.amazonaws.com/pages/articles/2019/01/07/86-300837.jpg.1080x505_q60_crop.jpg).
- Happy man with his copy of Windows 95 from [NBC News](https://media-cldnry.s-nbcnews.com/image/upload/t_fit-760w,f_auto,q_auto:best/newscms/2015_35/1190851/150824-windows-95-store-559p.jpg).
