...
We suspect that the mishandling of the log4shell vulnerability, by patching it in public on GitHub, likely contributed to the disaster.
This is a step-by-step guide for Open Source maintainers on how to handle the process properly, using the Log4Shell incident as a retrospective.
...
so let's put aside prevention for a moment and talk about what to do when you find out that you released something insecure.
...
Patching vulnerabilities is great, and finding one and contributing a fix can be pretty exciting. Unfortunately, the fact
