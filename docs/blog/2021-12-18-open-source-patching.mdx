...
We feel that the chaos which has followed the initial report of the Log4Shell vulnerability all started by how the public patching
...
Staying calm and knowing what to do when this occurs could save time and more importantly, damage to your reputation.
...
This is not a hit piece against anyone, but truly the core issue is that only a couple of
developers, who were donating their spare time to a project, had to issue a fix for one of the worst vulnerabilities in
recent history.
Log4j is a great case study because even though people had the best intentions, things went catastrophically wrong.

The world is built on open source software, and the next Log4Shell is inevitable.
We created a guide to help standardize a team's response when handling their next security issue, of any level of severity.
...
### Footnotes
* This data is collected from Contrast Security's scanned applications which belong to companies which use this service. This data
is skewed as is with all data, but we feel it gives a reasonable benchmark for all companies.
