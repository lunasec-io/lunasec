...
`2.17.0`. I wonder whether we should say "vulnerability" in quotes because
...
to these security disclosures with their ear to the ground anxiously wait for the latest Log4j version to be released. After the disclosure,
...
It requires the attacker to have a privileged position, at which point they would probably prefer a different attack vector to further
compromise a system.
...
With all the recent Log4j versions being released, knowing what guidance to follow in order to have a fully patched system can be quite confusing. Our [mitigation guide](https://www.lunasec.io/docs/blog/log4j-zero-day-mitigation-guide/) is continually updated with up-to-date guidance.
:::
...
for upgrading to `2.16.0` to have themselves fully covered.
...
We know from experience that this simply doesn't work.
