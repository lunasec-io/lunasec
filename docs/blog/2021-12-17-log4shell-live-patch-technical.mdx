...
7. When loading the Java code, all `static` methods are initialized (this is how most exploits are fired),
...
Once log4j has parses the message, the LDAP request is the first leg of the exploit that an attacker must implement in
order to attain RCE.
...
chain is fired off. The response from this request is the Java bytecode that contains that enables the actual RCE
vulnerability.

We won't show you the bytecode in the response because its binary data (and not very readable). Instead, we'll show you
the source code (
...
) for the bytecode that's sent back instead:
