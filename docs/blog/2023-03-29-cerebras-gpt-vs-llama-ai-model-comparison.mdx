...
performance. ([Link to press release]
...
much smaller model at 13B parameters. (Cerebras is ~6% of the size of GPT-3 and ~25% of the size of LLaMA's
full-size, 60B parameter model.)

That doesn't mean that it's useless though. As you'll see from the data released in the Cerebras paper, this model
is still a significant advancement over previous Open Source models like [GPT-2 (1.5B)](https://huggingface.co/gpt2),
[GPT-J (6B)]
(https://huggingface.co/EleutherAI/gpt-j-6B) or [GPT NeoX (20B)](https://huggingface.co/EleutherAI/gpt-neox-20b).

For anybody building around AI that doesn't want to depend on
[OpenAI's monopoly](https://www.lunasec.io/docs/blog/openai-not-so-open/), this is a welcome addition to the AI
landscape. (Even if it's only really an incremental step forward, as you'll see in a moment.)
...
<figcaption style={{fontSize:'small', color:'grey'}}>If you'd like to add data for a model, you can edit
