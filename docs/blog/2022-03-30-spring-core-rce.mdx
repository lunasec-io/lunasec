...
gotten very good at verifying that an exploit is a problem _before_ sound the alarm.

"It's an order of magnitude less work to tell lies than to refute them." - A former colleague

As developers and security engineers we're committed to only publishing the facts around what we know about the
situation, sharing our thoughts from analyzing the vulnerabilities, and giving you a way to
[stay informed](#get-notified-automatically) about new developments as they are discovered.
...
We've done enough analysis on this to be _concerned_ about the possibility that this is *maybe a problem*. We won't say
that it's _not an issue_ until we have a concrete understanding of the exploit scenarios. We believe that there _is_
some form of RCE in Spring core (i.e. Spring4Shell), but we just don't know if it's easily "remotely executable" or not
yet.
