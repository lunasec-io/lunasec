...
## Taming Chaos and Misinformation
...
We've done enough analysis on this to be _concerned_ about the possibility that this is a problem, so we're not going to
say that it's _not an issue_ until we have a concrete understanding of the exploit scenarios. We believe that there _is_
some form of RCE in Spring core, but we just don't know if it's easily "remotely executable" or not.

Code Execution by itself isn't terribly concerning. It's only concerning when an unauthorized user is able to *Remotely*
execute code. That's why we call then "Remote Code Execution" vulnerabilities and not just "Code Execution"! Log4Shell
was bad _because_ it was trivially exploitable *Remotely*.

Back in December we were the first team to write about the
[Java RCE vulnerability in log4j](https://www.lunasec.io/docs/blog/log4j-zero-day). We initially named that vulnerability
"Log4Shell" because, at the time, [CVE-2021-44228](https://nvd.nist.gov/vuln/detail/CVE-2021-44228) hadn't been published
nor had the Apache team published about any security notices.

We're calling this possible RCE "Spring4Shell" to help users search for it separately from the other RCE mentioned in
this post.

### Are you feeling Deja-vu?

Just as with Log4Shell back in December, misinformation spreads quickly and easily. It's much easier to photoshop a
screenshot showing an exploit and "redact the details" than it is to investigate a security vulnerability in a large
code base like Spring Core.

Fortunately, we're a team of Security Engineers and that's what we've been doing as pros for many years. We're happy to
dig in, understand it, and publish our analysis of the vulnerability for others to review.
