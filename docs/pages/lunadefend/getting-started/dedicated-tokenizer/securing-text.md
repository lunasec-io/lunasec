...
in the [type documentation](/pages/lunadefend/react-sdk/interfaces/SecurePropsLookup).
::::
## Secure Forms

Let's create a form secured with LunaDefend.  Here is a simple scenario where we want to tokenize a Social Security Number.
```tsx
...
`value` to update state. Where possible, LunaDefend elements attempt to mimic the behavior of basic DOM elements in order to make 
...
inspect it we can see the cross-domain iFrame that LunaDefend creates to protect your data.
:::
...
LunaDefend can work with component libraries that allow passing of a custom element, like MaterialUI.  Here is an example 
...
LunaDefend should also work just fine with a form library like Formik.
