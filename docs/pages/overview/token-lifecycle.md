...
### Tokenization
There are a few steps that happen behind the scenes when data gets tokenized.  This is what happens when
user clicks Submit on a form with a `<SecureInput>`.  For each element:
...
3. Your web app reads the token from the `<SecureInput>` in the same way it would read a change in a normal input.  
   You send the token to your server for storage with the rest of your normal data.
...
4. The Secure frame calls the Tokenizer to get a presigned URL to download the real data from s3.  The read grant is checked behind the scenes.
