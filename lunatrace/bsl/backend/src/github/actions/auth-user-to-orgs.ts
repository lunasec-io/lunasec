...
function getGithubOrgIdsFromApiResponse(userId: string, userGithubOrgs: GetUserOrganizationsQuery): string[] | null {
  ...
  if (!orgNodes) {
    defaultLogger.error(`orgNodes is null, api response from github is incomplete: ${userGithubOrgs.viewer.organizations.nodes}`);
    return null;
  }

  defaultLogger.debug(`[user: ${userId}] Github user's organizations count: ${orgNodes.length}`);
...
export async function authUserToOrgs(res: Response, userId: string) {
  ...
  defaultLogger.info(`[user: ${userId}] Collected Github user's organizations.`);
  ...
  defaultLogger.debug(`[user: ${userId}] Github user's organization ids: ${githubOrgIds}`);
