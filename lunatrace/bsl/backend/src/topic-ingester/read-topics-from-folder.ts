...
function parseTopic(topicPath: string, topicFolderName: string): Topic | void {
    ...
    if (!topicHasCorrectFiles(topicContents)){
        log.error({topicPath, topicContents}, 'Folder in topics missing required files to be a valid topic, skipping topic');
        return;
    }
...
function isMetadata(obj:unknown): obj is TopicMetadataWithSchema {
    return (
        typeof obj === 'object' && obj !== null && 'schemaVersion' in obj && !isNaN((obj as TopicMetadataWithSchema).schemaVersion)
    );
}
