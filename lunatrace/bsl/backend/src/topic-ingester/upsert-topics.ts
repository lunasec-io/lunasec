...
import {
    ...
    Topics_Insert_Input, Topics_Obj_Rel_Insert_Input, UpsertTopicsMutationVariables
...
export async function upsertTopics() {
    ...
    await Promise.all(topicData.map(async (t) => {
...
async function buildTopicVulnerabilities(cves: Topic['metadata']['cves']): Promise<Topics_Insert_Input['topic_vulnerabilities']| false> {
