...
ALTER
FUNCTION public.set_current_timestamp_updated_at() OWNER TO postgres;
...
AS
$$
begin execute format('create sequence if not exists public.project_builds_sequence_%s OWNED BY public.projects.id', translate(NEW.id::text, '-', '_'));
return NEW;
end $$;
...
AS
$$
begin NEW.build_number := nextval('public.project_builds_sequence_' || translate(NEW.project_id::text, '-', '_'));
RETURN NEW;
end $$;
...
AS
$$
begin execute format('create sequence if not exists public.build_scans_seq_%s OWNED BY public.scans.scan_number', translate(NEW.id::text, '-', '_'));
return NEW;
end $$;
...
AS
$$
begin NEW.scan_number := nextval('public.build_scans_seq_' || translate(NEW.build_id::text, '-', '_'));
RETURN NEW;
end $$;
