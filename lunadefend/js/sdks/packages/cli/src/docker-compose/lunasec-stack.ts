...
import { awsResourcesOutputFile, debug, outputDir } from '../constants/cli';
...
export class LunaSecStackDockerCompose {
  ...
  applicationBackEnd(): ComposeService {
    ...
    const awsResourcesPath = this.buildMountPath(outputDir);
  ...
  tokenizerBackEnd(): ComposeService {
    ...
    const localBuildConfig = {
      build: {
        context: '.',
        dockerfile: 'lunadefend/go/docker/tokenizerbackend.dockerfile',
        args: {
          tag: 'dev',
          version: version,
        },
  ...
  getAuthenticationProviders(): Record<string, AuthProviderConfig> | undefined {
    ...
    if (this.serviceCreationConfig.env === 'demo' || this.serviceCreationConfig.env === 'local-dependencies') {
  ...
  getFrontEndReactEnv(env: LunaSecStackEnvironment): LunaSecReactEnv {
    if (env === 'hosted-live-demo') {
      return {
    ...
    if (env === 'demo' || env === 'local-dependencies') {
      return {
