...
SECURE_FRAME_KEYSET_ARN=$(aws secretsmanager create-secret --name "secure-frame-keyset-$SETUP_ID" --secret-binary "fileb://$(pwd)/test_secrets/secure_frame_keyset" | jq .ARN)
SECURE_FRAME_SIGNING_KEYS_ARN=$(aws secretsmanager create-secret --name "secure-frame-signing-keys-$SETUP_ID" --secret-binary "fileb://$(pwd)/test_secrets/secure_frame_signing_keys" | jq .ARN)

echo "CIPHERTEXT_BUCKET_ARN=$CIPHERTEXT_BUCKET_ARN \
CIPHERTEXT_BUCKET_NAME=$CIPHERTEXT_BUCKET_NAME \
